...
class StackedLSTM(nn.Module):
    def __init__(self, args, n_feature, n_class):
        ...
        self.rnns, self.projections = [], []
        for i in range(args.layer):
            self.rnns.append(nn.LSTM(self.hidden_size, self.hidden_size, num_layers=1,
                                    dropout=dropout, batch_first=True, bidirectional=True))
            self.projections.append(nn.Linear(self.hidden_size * 2, self.hidden_size))

        self.out = nn.Linear(self.hidden_size, self.n_class)
